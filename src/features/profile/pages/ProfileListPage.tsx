...
/** Profile — paginated list view. */
export default function ProfileListPage() {
  const { page, pageSize, setPage } = useProfileStore();
  const [search, setSearch] = useState('');
  const debouncedSearch = useDebounce(search, 350);

  const { data, isLoading } = useProfile({ page, pageSize, search: debouncedSearch });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Profile"
        description="Manage profile records across your organization."
