...
import type { PaymentFormValues } from '../schemas/payments.schema';

const statusFilterOptions: SelectOption[] = [
  { label: 'All Status', value: '' },
  ...Object.values(PaymentStatus).map((value) => ({ label: value, value })),
];

const statusBadgeVariant: Record<PaymentStatus, 'success' | 'warning' | 'danger' | 'outline'> = {
  [PaymentStatus.Active]: 'success',
  [PaymentStatus.Pending]: 'warning',
  [PaymentStatus.Archived]: 'danger',
  [PaymentStatus.Inactive]: 'outline',
};
...
export default function PaymentsListPage() {
  ...
  const formModal = useModal<Payment>();
  const viewModal = useModal<Payment>();
  const deleteModal = useModal<Payment>();
  const editing = formModal.data;

  const [selectedIds, setSelectedIds] = useState<Set<Payment['id']>>(new Set());
  ...
  const toggleSelect = useCallback((id: Payment['id']) => {
    setSelectedIds((prev) => {
  ...
  const handleSubmit = (values: PaymentFormValues) => {
    ...
    if (editing) {
      updatePayment.mutate({ id: editing.id, dto: values }, { onSuccess });
    } else {
      createPayment.mutate(values, { onSuccess });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        title="All Payments"
        headerActions={
          <PermissionGate permissions={[PAYMENTS_PERMISSIONS.CREATE]}>
      ...
      <Modal
        ...
        description="Provide the payment's details below."
      >
        <PaymentForm
      ...
      <Modal
        ...
        description="Read-only view of this payment's record."
      >
        {viewModal.data && (
          <PaymentForm defaultValues={viewModal.data} readOnly onSubmit={() => {}} />
        )}
      </Modal>
      ...
      <ConfirmDialog
        ...
        onConfirm={() =>
          deleteModal.data &&
          deletePayment.mutate(deleteModal.data.id, { onSuccess: deleteModal.close })
        }
